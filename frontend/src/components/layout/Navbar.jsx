...
import {
  ...
  FiPackage, FiSettings, FiChevronDown, FiSearch, FiHeart
} from 'react-icons/fi';
...
export default function Navbar() {
  ...
  return (
    <>
      {/* Announcement bar */}
      <div className="bg-rose-600 text-white text-xs py-2 text-center font-medium tracking-wide">
        🎁 Free shipping on orders above ₹999 &nbsp;|&nbsp; Bulk orders available — WhatsApp us!
      </div>

      <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-[0_2px_20px_rgba(0,0,0,0.08)]' : 'bg-white/98 backdrop-blur-sm border-b border-rose-50'}`}>
          ...
          <div className="flex items-center justify-between h-16 gap-4">
            ...
            <Link to="/" className="flex items-center gap-2.5 flex-shrink-0">
              ...
              <div className="leading-none">
                <p className="font-display font-bold text-rose-700 text-base tracking-tight">KIKI'S</p>
                <p className="text-[9px] text-gray-400 uppercase tracking-[0.22em] font-medium">Return Gifts</p>
              </div>
            </Link>
            ...
            <nav className="hidden md:flex items-center gap-1">
              {[{ to: '/', label: 'Home' }, { to: '/shop', label: 'Shop' }].map((l) => (
                <NavLink key={l.to} to={l.to} end={l.to === '/'}
                  className={({ isActive }) =>
                    `px-4 py-2 rounded-full text-sm font-medium transition-all ${isActive ? 'text-rose-600 bg-rose-50' : 'text-gray-600 hover:text-rose-600 hover:bg-rose-50'}`
                  }>
                  {l.label}
                </NavLink>
              ))}
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button onClick={() => setSearchOpen(true)}
                className="p-2.5 text-gray-500 hover:text-rose-600 hover:bg-rose-50 rounded-full transition-all">
                <FiSearch size={19} />
              </button>

              {/* Cart */}
              <Link to="/cart" className="relative p-2.5 text-gray-500 hover:text-rose-600 hover:bg-rose-50 rounded-full transition-all">
                <FiShoppingCart size={19} />
                <AnimatePresence>
                  {cartCount > 0 && (
                    <motion.span key="badge" initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}
                      className="absolute -top-0.5 -right-0.5 bg-rose-600 text-white text-[10px] w-4.5 h-4.5 w-5 h-5 rounded-full flex items-center justify-center font-bold leading-none">
              ...
              {user ? (
                <div className="relative">
                  <button onClick={() => setUserMenuOpen(!userMenuOpen)}
                    className="flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-full hover:bg-rose-50 transition-all border border-transparent hover:border-rose-100">
                    <div className="w-7 h-7 bg-rose-100 rounded-full flex items-center justify-center">
                      <span className="text-rose-700 font-bold text-xs">{user.name[0].toUpperCase()}</span>
                    </div>
                    <span className="hidden sm:block text-sm font-medium text-gray-700 max-w-[80px] truncate">{user.name.split(' ')[0]}</span>
                    <FiChevronDown size={13} className={`text-gray-400 transition-transform ${userMenuOpen ? 'rotate-180' : ''}`} />
                  </button>
                  <AnimatePresence>
                    {userMenuOpen && (
                ...
                <Link to="/login" className="btn-primary py-2 px-5 text-xs hidden sm:flex">Login</Link>
              )}

              {/* Mobile menu */}
              <button className="md:hidden p-2 text-gray-600 hover:bg-rose-50 rounded-full" onClick={() => setMenuOpen(!menuOpen)}>
              ...
              <div className="page-container py-4 flex flex-col gap-1">
                {[{ to: '/', label: 'Home' }, { to: '/shop', label: 'Shop' }].map((l) => (
                  <NavLink key={l.to} to={l.to} end={l.to === '/'} onClick={() => setMenuOpen(false)}
                    className={({ isActive }) => `text-sm font-medium py-2.5 px-3 rounded-xl ${isActive ? 'text-rose-600 bg-rose-50' : 'text-gray-700'}`}>
                    {l.label}
                  </NavLink>
                ))}
                {!user && (
