...
export const getInventoryMovements = asyncHandler(async (req, res) => {
  ...
  const [orders, offlineSales] = await Promise.all([
    orderIds.length
      ? Order.find({ _id: { $in: orderIds } }).select('customerName customerAddress orderNumber invoiceNumber createdAt')
      : [],
  ...
  const enrichedMovements = movements.map((movement) => {
    ...
    if (plainMovement.referenceModel === 'Order' && plainMovement.referenceId) {
      ...
      referenceNumber = order?.invoiceNumber || order?.orderNumber || null;
    }
